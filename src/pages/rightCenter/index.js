...
const rightCenter = function (props) {
  ...
  const [rightEchartCen, setRightEchartCen] = useState([]);
  const [returnAps, setReturnAps] = useState({});
  const [aps, setAps] = useState({});
  const [fourWeekOption, setFourWeekOption] = useState({});
  ...
  useEffect(() => {
    if (rightEchart?.length) {
      init();
      const cenData = rightEchart.slice(0, 6).map((item, index) => {
        return {
          ...item,
          delay: 3 * (index + 1),
        };
      });
      //console.log(cenData, 'cenData--cenData')
      setRightEchartCen(cenData);
      rightEchartCenScroll(12000, cenData);
    }
    return init;
  }, [rightEchart]);
  const init = () => {
  ...
  const rightEchartCenScroll = (time, data) => {
    timer.current = setTimeout(() => {
      ...
      setRightEchartCen(
        loopData(rightEchart, 6).map((item, index) => {
          return {
            ...item,
            delay: 3 * (index + 1),
          };
      ...
      rightEchartCenScroll(12000, data);
    }, time);
  };
  let currentPage = 0;
  ...
  useEffect(() => {
    ...
    setFourWeekOption(fourWeekOption);
  }, [fourWeekUtilizationRate]);
  useEffect(() => {
    const fourAps = {
      tooltip: {
      ...
      <Row>
        <Col span={8} className="right-center-part1">
          <div id="review_box_right">
            <ul>
              {rightEchartCen.map((item, index) => {
                return (
                  <li key={index} className="run-list" style={{ animationDelay: item.delay + 's' }}>
                    <div
                      className={
